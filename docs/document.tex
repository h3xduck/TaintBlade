...
although it could be extended to work in Linux easily in a future. It must also
be noted that we decided to develop the tainting system from the ground -
instead of using an existing solution - because we could not find any
...
Triton or Tytan but, as indicated in table \ref{}, we identified certain
drawbacks in each of them. Therefore, we decided to create a new 64-bit
compatible multi-color tainting engine that integrates with Intel PIN (it
relyies on the instrumentation module we described previously).
