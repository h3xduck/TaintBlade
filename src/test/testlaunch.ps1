...
function runTest {
    ...
    $testProcess = Start-Process cmd -ArgumentList '/k', "..\..\external\pin-3.25-98650-g8f6168173-msvc-windows\pin-3.25-98650-g8f6168173-msvc-windows\pin.exe -follow_execv -t ..\..\PinTracer\Win32\Release\PinTracer.dll -o pinlog.dfx -s syspinlog.dfx -i imgpinlog.dfx -d debuglogfile.dfx -test testset.txt -taint taintsources.txt -- ..\..\..\samples\$programInfo" -PassThru
    $user32 = New-Object User32
    $null = $user32::SetForegroundWindow((Get-Process -id $pid).MainWindowHandle)
