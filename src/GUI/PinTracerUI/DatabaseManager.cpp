...
void DatabaseManager::buildTaintEventsTree(QTreeWidget *treeWidget)
{
    ...
    treeWidget->setHeaderLabels(headers);
    QSqlQuery query;
    query.exec("SELECT * from taint_events as te JOIN taint_routines as tr ON (te.routine_idx = tr.idx) JOIN dll_names AS d ON (tr.dll_idx = d.idx);");

    while(query.next())
    {
        //The first level will only show the dll
        qDebug("Received item");
        QTreeWidgetItem *item = new QTreeWidgetItem();
        item->setText(0, query.value("dll_name").toString());

        //Second level will show the functions (routines)
        //Child showing headers for second level
        QTreeWidgetItem *childHeader = new QTreeWidgetItem();
        childHeader->setText(0, "Function");
        childHeader->setText(1, "Entry base address");
        childHeader->setText(2, "Length");
        childHeader->setText(3, "String value");
        item->addChild(childHeader);
        int numArgs = query.value("num_args").toInt();
