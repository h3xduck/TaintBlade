...
PROTOCOL::ByteBufferPushButton::ByteBufferPushButton(QString text, QWidget* parent, int byteIndex, int protocolElementIndex) : QPushButton(text, parent)
{
	this->startByte_ = byteIndex;
	this->endByte_ = byteIndex;
	this->textList_.append(text);
	this->protocolElementIndex_ = protocolElementIndex;
	this->type_ = PROTOCOL::ByteBufferPushButton::TNONE_CNONE;
	this->setFixedSize(30, 90);
	this->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
	this->setText(this->textList_.join(" "));
