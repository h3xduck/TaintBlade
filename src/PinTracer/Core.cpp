...
void printInstructionOpcodes(void *ip, std::string inst_assembly, uint32_t inst_size)
{
    std::stringstream tmpstr;
    tmpstr << std::hex << ip;
    uint8_t opcodes[15];
    PIN_SafeCopy(opcodes, ip, inst_size);
    *out << tmpstr.str() <<"\t";
    for(uint32_t ii=0; ii<inst_size; ii++)
    {
        *out << std::setfill('0') << std::setw(2) << std::hex << static_cast<int>(opcodes[ii]) << " ";
    }
    *out << "\t" << inst_assembly << std::endl;
}
...
VOID SyscallTrace(THREADID threadIndex, CONTEXT *ctx, SYSCALL_STANDARD std, VOID *v)
{
    ...
    *sysinfo_out << std::dec << "SYSCALL " << (int)syscall_number << std::endl;
    ADDRINT syscall_args [4];
    syscall_args[0] = PIN_GetSyscallArgument(ctx, std, 0);
    syscall_args[1] = PIN_GetSyscallArgument(ctx, std, 1);
    syscall_args[2] = PIN_GetSyscallArgument(ctx, std, 2);
    syscall_args[3] = PIN_GetSyscallArgument(ctx, std, 3);
    for(int ii=0; ii<4; ii++)
    {
        *sysinfo_out << "ARG " << ii << ":\n" << syscall_args[ii] << std::endl;
    }
    *sysinfo_out << "\n";

    sys_parser.print_syscall_attempt(sysinfo_out, syscall_number, syscall_args);
}
...
int main(int argc, char* argv[])
{
    ...
    {
        sysinfo_out = new std::ofstream(sysinfo_filename.c_str());
    }
