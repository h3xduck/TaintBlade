...
public:
	...
	enum taint_lead_class
	{
		NONE,
		TAINT_LEAD_SINK //Was sent to CreateProccess or similar
	};
	typedef struct color_taint_lead_t
	{
		taint_lead_class leadClass;
		struct taint_lead_sink_data_t {
			std::string dllName;
			std::string funcName;
...
private:
	...
	/*
	Stores colors, and a list of 'leads' which indicate special places this color is involved in (e.g.: they were sent into a taint sink)
	If a color does not appear here, it did not take part in any special operation
	*/
	std::tr1::unordered_map<UINT16, color_taint_lead_t> colorLeadMap;
...
public:
	...
	*/
	...
	/**
	Stores in the map of taint color leads a value
	*/
	void logColorTaintLead(UINT16 color, TagLog::color_taint_lead_t lead);
	...
	*/
	...
	/**
	Get vector of color taint leads
	*/
	std::vector<std::pair<UINT16, color_taint_lead_t>> getColorsLeadsVector();

	/**
	Get the leads of a color, returns NONE lead if none
	*/
	color_taint_lead_t getColorTaintLead(UINT16 color);
