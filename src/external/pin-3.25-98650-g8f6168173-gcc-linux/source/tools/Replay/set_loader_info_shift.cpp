...
static VOID TraceImageLoad(IMG img, VOID* v)
{
    ...
    else if (IMG_Name(img) == "SecondImage")
    {
        ...
        // Check that RTN_FindByName works for RTN created with RTN_CreateAt
        RTN fakeRTN = RTN_FindByName(img, "FakeRtn");
        ASSERT(RTN_Valid(fakeRTN), "Could not find the expected routine");
    }
