...
VOID CONTROL_MANAGER::InitKnobs()
{
    ...
    _control_rep_nonstop_knob = new KNOB< BOOL >(KNOB_MODE_WRITEONCE, _control_family, "controller-rep-nonstop", "0",
                                                 "Do not stop on repeat string instructions", _prefix);
    ...
    _control_input_file_knob = new KNOB< string >(KNOB_MODE_WRITEONCE, _control_family, "controller-input-file", "",
                                                  "file name to write controller alarms", _prefix);
}
...
CONTROL_MANAGER::CONTROL_MANAGER(const string prefix, const string family, const string description)
{
    ...
    _control_ssc_code_exclude = FALSE;
}
...
VOID CONTROL_MANAGER::AfterForkInChildCallback(THREADID tid, const CONTEXT* ctxt, VOID* v)
{
    CONTROL_MANAGER* mngr = static_cast< CONTROL_MANAGER* >(v);
    if (mngr->_control_log_knob->Value())
    {
        // close the controller event log
        mngr->_out.close();

        // Get original file name
        string filename = mngr->_control_log_file_knob->Value();
        bool use_txt    = filename.find(".txt") != string::npos;

        // Remove .txt from file name
        if (use_txt)
        {
            string::size_type i = filename.size();
            string::size_type p = filename.rfind(".txt", i);
            if ((i - p) == 4) filename = filename.substr(0, p);
        }

        // We are creating a unique log file for every process by
        // adding the pid to the output file name
        string ppidext = "";
#if defined(TARGET_LINUX) || defined(TARGET_MAC)
        ppidext = decstr(getppid()) + "-";
#endif
        filename = filename + "." + ppidext + decstr(getpid());
        if (use_txt)
        {
            filename = filename + ".txt";
        }

        // open the controller event log again
        mngr->_out.open(filename.c_str());
    }
}
...
VOID CONTROL_MANAGER::RegisterHandler(CONTROL_HANDLER ch, VOID* val, BOOL passContext, CONTROL_HANDLER late_ch)
{
    ...
    params.has_late_handler = (late_ch != NULL);
    ...
    // PIN_ExecuteAt which terminates a Trace and is precise.
    CONTROL_HANDLER_PARAMS late_params;
    late_params.val              = val;
    late_params.passContext      = passContext;
    late_params.has_late_handler = false;
    if (late_ch)
    {
        late_params.handler = late_ch;
    }
    else
    {
        late_params.handler = ch;
    }

    // Always set late handler flag
    // If there is no special late handler then
    // call original handler as late handler.
    // Done in order to synchronize PinPlay with SDE.
    _late_control_handler.push_back(late_params);
    if (!_late_handler)
    {
        _late_handler = TRUE;
...
VOID CONTROL_MANAGER::Activate()
{
    ...
    // Handle controller input file
    UINT32 input_file_chains = 0;
    if (_control_input_file_knob->Value() != "")
    {
        //open the controller input file
        string filename = _control_input_file_knob->Value();
        _in.open(filename.c_str());
        ASSERT(_in.is_open(), "could not open controller input file: " + filename);
        ...
        while (true)
        {
            ...
            // Read all records and create chains from them
            static const UINT32 BUFSIZE = 200;
            CHAR chain_record[BUFSIZE + 1];
            _in.getline(chain_record, BUFSIZE);
            string chain_str(chain_record);

            if (chain_str == "" || chain_str == " ") break;

            // First letter '#' : this is a comment
            if (chain_str[0] == '#') continue;

            if (_control_log_knob->Value())
            {
                // Emit the controller knobs
                _out << "Input file Controller knob " << input_file_chains << " : " << chain_str << endl;
            }
    ...
    if (AddOldKnobs())
    {
        ...
        if (regionControllerNum > 0)
        {
            ASSERT(FALSE, "Illegal usage of old controller knobs together with -regions:in or "
                          "external regions ");
        }
    }
...
#if defined(TARGET_LINUX) || defined(TARGET_MAC)
    PIN_AddForkFunction(FPOINT_AFTER_IN_CHILD, AfterForkInChildCallback, this);
#endif
}

VOID CONTROL_MANAGER::Fire(EVENT_TYPE eventID, CONTEXT* ctxt, VOID* ip, THREADID tid, BOOL bcast, const string& alarm_str,
                           VOID* event_handler, CONTROL_CHAIN* chain, UINT32 alarm_id)
{
    if (_control_log_knob->Value())
    {
        //emit the controllers events log
        _out << " TID" << tid << ":  event type: " << _events.IDToString(eventID) << " Alarm string: " << alarm_str
    ...
    // Check if this alarm can support late handler
    BOOL canSupportLateHandler = FALSE;
    if (chain && chain->CanSupportLateHandler(alarm_id)) canSupportLateHandler = TRUE;
    ...
    {
        // If there are 2 handlers (standard and late) then call both of them.
        // However if there is only one handler then call the standard handler
        // in the call order of the late handler.
        // This is done in order to align SDE and PinPlay controller handling.
        // If we are using default start event then always call it.
        if (iter->has_late_handler || (!iter->has_late_handler && !canSupportLateHandler) || HasStartEvent() == FALSE)
        {
...
BOOL CONTROL_MANAGER::HasSkipInt3()
{
    list< CONTROL_CHAIN* >::iterator iter = _control_chain.begin();
