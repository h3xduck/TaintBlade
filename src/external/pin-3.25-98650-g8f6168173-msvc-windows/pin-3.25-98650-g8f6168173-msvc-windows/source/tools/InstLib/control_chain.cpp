                         ...
                         const string& alarm_str)
{
    ...
    {
        _control_mngr->Fire(eventID, ctx, ip, tid, bcast, alarm_str, _event_handler, this, alarm_id);
    }
...
VOID CONTROL_CHAIN::Parse(const string& chain_str)
{
    ...
    for (UINT32 i = 0; i < control_str.size(); i++)
    {
        ...
        {
            // If previous alarm in the chain contain repeat or name or waitfor tokens
            // then it is not legal to create another alarm in the chain
            if (i > 0)
            {
                if (_repeat_token == REPEAT_INDEFINITELY)
                {
                    PIN_ERROR("No more alarms are allowed after repeat token");
                }
                if (_name != "")
                {
                    PIN_ERROR("No more alarms are allowed after name token");
                }
                if (_wait_for_id != NO_WAIT)
                {
                    PIN_ERROR("No more alarms are allowed after waitfor token");
                }
            }
...
BOOL CONTROL_CHAIN::HasSkipInt3()
{
    for (UINT32 i = 0; i < _alarms.size(); i++)
    {
        if (_alarms[i]->GetAlarmTypeFromManager() == ALARM_TYPE_INT3)
        {
            return TRUE;
        }
    }
...
VOID CONTROL_CHAIN::SetLateHandler()
{
    for (UINT32 i = 0; i < _alarms.size(); i++)
    {
        if (_alarms[i]->CanSupportLateHandler()) _alarms[i]->SetLateHandler();
    }
}

BOOL CONTROL_CHAIN::CanSupportLateHandler(UINT32 alarm_id) { return _alarms[alarm_id]->CanSupportLateHandler(); }
