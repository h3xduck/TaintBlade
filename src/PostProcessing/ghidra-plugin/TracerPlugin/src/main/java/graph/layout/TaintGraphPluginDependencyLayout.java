...
/**
 * A custom layout to arrange the plugin vertices of the {@link TaintGraphPlugin}, using
 * the number of dependencies as a guide for arrangement.
 */
public class TaintGraphPluginDependencyLayout
		extends AbstractVisualGraphLayout<TaintVertex, TaintEdge> {

	protected TaintGraphPluginDependencyLayout(TaintGraph graph, String name) {
		super(graph, name);
	}

	@Override
	public TaintGraph getVisualGraph() {
		return (TaintGraph) getGraph();
	}

	@Override
	public AbstractVisualGraphLayout<TaintVertex, TaintEdge> createClonedLayout(
			VisualGraph<TaintVertex, TaintEdge> newGraph) {
		if (!(newGraph instanceof TaintGraph)) {
			throw new IllegalArgumentException("Must pass a " + TaintGraph.class.getSimpleName() +
				"to clone the " + getClass().getSimpleName());
		}

		TaintGraphPluginDependencyLayout newLayout =
			new TaintGraphPluginDependencyLayout((TaintGraph) newGraph, getLayoutName());
		return newLayout;
	}

	@Override
	protected GridLocationMap<TaintVertex, TaintEdge> performInitialGridLayout(
			VisualGraph<TaintVertex, TaintEdge> g) throws CancelledException {

		GridLocationMap<TaintVertex, TaintEdge> results = new GridLocationMap<>();
		...
		Map<Integer, List<TaintVertex>> verticesByRow =
			LazyMap.lazyMap(new HashMap<>(), () -> new LinkedList<>());

		for (TaintVertex v : vertices) {
			Collection<TaintEdge> in = g.getInEdges(v);
			Collection<TaintEdge> out = g.getOutEdges(v);
		...
		for (int i = rowValues.size() - 1; i > 0; i--) {
			...
			List<TaintVertex> rowVertices = verticesByRow.get(row);
			...
			for (int j = 0; j < columnCount; j++) {
				TaintVertex v = rowVertices.get(j);
		...
		List<TaintVertex> lastRow = verticesByRow.get(0); // 0 since they have no edges
	...
	private class VertexNameComparator implements Comparator<TaintVertex> {
